...
if game_var == 1:
    ...
    if rand % 2 != 0:
        ...
        print(f'Первым ходит игрок по имени {first_name}.\nВаш ход.\nНа столе лежит {num} конфет(а). За один ход можно забрать не более чем 28 конфет')
    else:
        player_1 = second_name
        player_2 = first_name
        print(f'Первым ходит игрок по имени {second_name}. Ваш ход. На столе лежит {num} конфета. За один ход можно забрать не более чем 28 конфет')
    ...
    while num > 0:
        ...
        if 0 < step < 29 and step <= num:
            num -= step
            count += 1
            print(f'На столе осталось {num} конфет(а)')
        elif step >= 29 or step == 0:
    ...
    if rand % 2 != 0:
        ...
        player_1 = second_name
        player_2 = first_name
        print(f'Первым ходит игрок по имени {second_name}. Ваш ход. На столе лежит {num} конфет(а). За один ход можно забрать не более чем 28 конфет')
    ...
    if player_1 != 'Компьютер':
        while num > 0:
            step = int(input('Введите, сколько нужно забрать конфет: '))
            while step > 28 or step == 0 or step > num:
                step = int(input('Неверное число конфет. Попробуйте еще раз.\nВведите, сколько нужно забрать конфет: '))
            if 0 < step < 29 and step <= num:
                num -= step
                count += 1
                print(f'На столе осталось {num} конфет(а)')
            ...
            if num > max_step and num > 0 :
                ...
                num -= step
                count += 1
                print(f'Компьютер взял {step} конфет. На столе осталось {num} конфет(а).')
